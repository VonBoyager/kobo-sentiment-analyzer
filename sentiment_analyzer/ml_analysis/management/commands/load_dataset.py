...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            # Check for recommended review_date column
            if 'review_date' not in df.columns:
                self.stdout.write(
                    self.style.WARNING('⚠️  WARNING: "review_date" column not found. All records will use current upload time as submission date.')
                )
        ...
        section_mapping = {
            ...
            'Culture & Values': ['mission_values_meaningful', 'positive_inclusive_culture', 'company_values_alignment'],
            'Diversity & Inclusion': ['diversity_hiring_promotion', 'employees_feel_included', 'respectful_welcoming_environment'],
        ...
        for i in range(0, len(df), batch_size):
            ...
            with transaction.atomic():
                for _, row in batch_df.iterrows():
                    try:
                        # Parse review_date if present, otherwise use current time
                                    ...
                                    if parsed_date:
                                        ...
                                        try:
                                            ...
                                            if pd.notna(parsed_date):
                                                submitted_at = timezone.make_aware(parsed_date.to_pydatetime())
                                        except:
                                            self.stdout.write(
                                                self.style.WARNING(f'Could not parse date: {review_date_str}, using current time')
                                            )
                            except Exception as e:
                                self.stdout.write(
                                    self.style.WARNING(f'Error parsing review_date: {e}, using current time')
                                )
