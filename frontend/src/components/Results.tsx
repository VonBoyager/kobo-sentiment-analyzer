...
import { TrendingUp, TrendingDown, Sparkles, MessageSquare, Award, Search, Filter, ZoomIn, ZoomOut, RotateCcw } from 'lucide-react';
...
export function Results() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('/api/ml/results-data/');
        if (!response.ok) {
  ...
  if (loading) {
    return <div className="text-center p-8 text-white">Loading results...</div>;
  }
        ...
        <div className="lg:col-span-2 bg-gradient-to-br from-blue-900/20 to-indigo-900/20 rounded-2xl p-6 sm:p-8 border border-blue-800 hover:shadow-md transition-shadow">
          ...
          {data?.overall_company_score !== undefined ? (
            ...
            <div className="text-gray-400 text-center py-8">
              No company score available. Upload CSV data and train models to see results.
            </div>
          )}
        </div>
        ...
        <div className="bg-gray-800 rounded-2xl p-6 sm:p-8 border border-gray-700 hover:shadow-md transition-shadow">
          ...
          {normalizedFeatureImportance.length > 0 ? (
            ...
            <div className="text-gray-400 text-center py-12">
              No feature importance data available. Train models to see feature importance.
            </div>
          )}
        </div>
          ...
          {filteredWords.length > 0 ? (
                  ...
                  {filteredWords.map((wordData, idx) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          // Optional: Could show a modal with more details
                        }}
                      >
            ...
            <div className="text-gray-400 text-center py-12">
              {allTrendingWords.length === 0 
                ? 'No word data available. Upload CSV data and train models to see trending words.'
